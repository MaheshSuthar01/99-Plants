    ...
    public partial class RecoverPassword : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
...

            {
                ...
                {
                    Response.Redirect("~/Main_page/Default.aspx");
                }
            }
            if(!IsPostBack)
            {
                if(dt.Rows.Count!=0)
                {
                    txtNewConPass.Visible = true;
                    txtNewPass.Visible = true;
                    lblNewPass.Visible = true;
                    lblNewConPass.Visible = true;
                    btnResetPass.Visible = true;
                }
                else
                {
                    lblmsg.Text = "Your Password Reset Link is Expired or Invaild...try again";
